...
def plans(db: DbSession, response: Response, settings: AppSettings):
    response.headers['Cache-Control'] = 'no-store'
    return service.catalogue(db, settings)


@router.get('/enrolment-plans', response_model=PublicPlansRead, response_model_exclude_unset=True)
def enrolment_plans(actor: CurrentUser, db: DbSession, response: Response, settings: AppSettings):
    response.headers['Cache-Control'] = 'no-store'
    return service.enrolment_catalogue(db, actor.id, settings)
