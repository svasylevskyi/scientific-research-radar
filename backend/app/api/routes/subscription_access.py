...
def mine(actor: CurrentUser, db: DbSession, settings: AppSettings, digest_id: UUID | None = None, run_id: UUID | None = None):
    ...
    if digest_id:
        ...
        _, issues = service.assess(db, actor.id, digest.maximum_papers, settings=settings, digest_id=digest_id)
        ...
        if run_id:
            ...
            _, issues = service.assess(db, actor.id, run.digest_snapshot['maximum_papers'], str(run.trigger), context, settings, digest_id=digest_id)
...
def overview(user_id: UUID, actor: CurrentAdmin, db: DbSession, settings: AppSettings,
    ...
    data = {**service.overview(db, user_id, settings), 'email': user.email,
...
class FreeDigestSelection(BaseModel):
