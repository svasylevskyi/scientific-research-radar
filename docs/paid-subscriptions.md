...
7. After checkout, use **Manage billing** for the existing configured portal (payment-method updates, invoices and cancellation). Return goes to `/radar/subscription`. Portal plan/quantity updates must remain disabled. Existing customers can still manage billing if an admin later restores complimentary access.
...
### Cancellation and automatic Free fallback

This increment adds **Cancel at renewal** on Subscription and usage. Radar opens a Stripe-hosted confirmation flow for the authenticated subscriber's existing subscription. Opening or returning from the portal does not confirm cancellation: verified Stripe observations remain authoritative. The existing portal configuration must have cancellation enabled with **mode `at_period_end`**, and plan updates disabled. Radar now refuses incompatible portal configurations. No new provider permissions or secrets are required.

Verified paid access continues through its paid period, including an annual period. At a known cancellation deadline or observed terminal cancellation, Free becomes effective. Incomplete initial payment leaves Free available. A failed renewal retains the existing three-day grace rule when prior settlement qualifies; after grace, or for a verified failed payment without grace, Free applies. Unverified invoices, mismatched prices and unknown synchronization failures do not independently trigger a new fallback. Previously established Free fallback remains available until verified paid recovery. Free fallback does not cancel payment collection, erase invoices, create a Stripe Free subscription, or permit a duplicate paid checkout while an existing subscription is nonterminal.

**Retained research:** every digest and result remains readable/editable/deletable under existing ownership and active-run safeguards. On Subscription and usage, users can choose the digests to keep active under their pinned Free tier, before cancellation or afterward. The default is the most recently successfully run digest(s), falling back to last edit/creation, with deterministic ordering. Excess digests are retained but new runs/retries are denied. Deleting a selected digest automatically fills the available slot from retained digests. The default Free limit is one, but the selection respects the administrator-configured pinned limit.

**Schedules:** incompatible schedules are paused with settings retained and the dispatch cursor cleared. Pauses are materialized by subscription reads, billing reconciliation, or scheduler admission, without a dependency on the user visiting the page. Existing accepted runs and their deliveries finish. Recovery does not silently reactivate paused schedules: review and save each eligible schedule to resume from its next future occurrence, with no catch-up run. The schedule preview explains the pause even if its previous cursor is empty.

**Allowances:** migration `20260913_0023` introduces an account-level monthly clock and retained Free preferences. Existing accounts keep their latest established paid anchor where available, otherwise their Free anchor. New accounts retain the clock initialized from their Free enrollment even when subsequently purchasing paid access. Billing renewal dates remain Stripe-controlled; the account allowance clock is separate. Cancel/fallback/recovery do not reset it. Usage is summed across checkout identities, including reservations; legacy usage windows overlapping the current account window are counted conservatively until old windows expire. No historical usage records are rewritten. Thus a user who already spent more than Free permits has zero remaining until reset.

Normal deployment applies the migration to API, research and scheduler processes. Complimentary access remains unchanged. No live payments, immediate upgrades, paid downgrades, interval changes, prorated invoices or billing notification emails are introduced here.

Acceptance:
1. Set the existing Stripe portal cancellation mode to **At end of billing period** and deploy. Open Cancel at renewal and confirm in Stripe; refresh/reconcile to verify the scheduled cancellation and effective date. Before the deadline, paid access remains available. The general portal can be used to review/revoke cancellation before it takes effect.
2. Choose Free digests with excess saved digests present. Simulate the period-end cancellation in sandbox: only selected digests accept new research, all old results remain readable, and usage does not reset.
3. Exercise failed renewal with prior paid coverage, grace expiry, and successful recovery. Confirm outstanding payment information remains separate from Free entitlement.
4. Confirm due schedules pause even without opening the account page. After payment recovery, review/save a schedule and verify its next dispatch is future-only.
5. Confirm unauthorized selection, inactive digest retries, duplicate webhook delivery and repeated fallback/recovery cannot bypass quotas or restart schedules.

Stripe reference: [portal cancellation flows](https://docs.stripe.com/customer-management/portal-deep-links). Rollback of the schema refuses while paused schedules exist, to prevent silently reactivating them; preserve the compatible schema or restore a reviewed backup.

### Scheduled downgrades, billing interval switches, and notifications

Subscribers can now schedule one paid downgrade or an interval-only monthly/yearly switch in **Subscription and usage → Changes at renewal**. Both directions of an interval switch take effect at the **end of the current paid billing period**: annual-to-monthly waits until annual renewal. No immediate payment, partial-period credit, or proration is requested. The review dialog shows the immutable target revision's recurring price, date, limits, and active digests. Same-tier interval switches retain the purchased revision, including grandfathered terms; they do not silently move to the latest catalogue revision.

A cross-tier downgrade must be a currently published, reviewed, mapped paid revision, in the same currency, with a lower monthly catalogue price and no increase in any quota, schedule capability, or email entitlement. The selected interval's normalized price cannot exceed the subscriber's current normalized price. Ambiguous cross-grade/upgrade combinations are excluded from this increment. Free remains the separate cancellation-at-renewal flow. Trial, discounted, automatic-tax, multi-item, manually collected, pending-update, externally scheduled, or otherwise unsupported billing configurations require operator review.

**Execution and undo:** Radar durably records an intent before creating a Stripe Subscription Schedule from the existing subscription. It preserves the current phase and configures the target phase at renewal with proration disabled, `billing_cycle_anchor=phase_start`, and `end_behavior=release`. Timeouts retry identical persisted parameters and idempotency keys. Only one unresolved intent per owner is allowed, serialized by the existing database account lock. An unknown create result older than 23 hours is never replayed. **Undo scheduled change** releases the schedule while keeping the subscription; undo closes 30 seconds before renewal to avoid initiating a boundary race. A post-release canonical refresh verifies whether undo actually won. There is no automatic reversal of an already applied price change.

The worker and existing subscription/invoice webhook reconciliation recognize only the saved target price, at or after its saved effective date, and a matching owner-scoped, tagged schedule. The purchased binding changes on verified phase entry; invoice settlement still controls paid access. Historical invoices are checked against the binding that applied to their service period. Initial or renewal invoices remain required; a non-prorated `subscription_update` invoice is accepted only at a verified schedule boundary. Failed renewal uses the existing three-day grace and Free fallback, with recovery after verified payment. A schedule is released after successful payment so later changes can be requested.

**Retained work:** Select the digests that remain active when the lower tier has fewer slots; excess digests and completed research remain readable/editable. Active paid digests can also be changed after downgrade. Incompatible schedules (including oversized paper limits) are paused and retain their saved definitions. They resume only after explicit review/save, from a future occurrence. Already accepted runs finish. Account monthly allowance anchors and usage do not reset; switching intervals does not grant a fresh month of quota. Overlapping legacy usage remains conservatively counted until its recorded window expires.

**Notifications:** Scheduling, undo, completion after verified payment, interrupted changes, cancellation/withdrawal, payment failure, Free fallback, and paid recovery create transactional outbox entries. They appear on Subscription and usage and are delivered by the existing API billing worker through configured SMTP to the Radar account email. Failed SMTP delivery retries with bounded backoff (eight attempts), a stable Message-ID, and a five-minute crash-recovery lease. A final email failure remains visible in Radar. Event deduplication prevents ordinary webhook/reconciliation replay from generating duplicate notices; SMTP itself is at-least-once and a provider-acceptance timeout can still produce duplicate email. Earlier historical lifecycle events are not comprehensively backfilled.

**Deployment:**

1. Grant the existing restricted sandbox API key **Subscription Schedules: Write** (including retrieval/release), in addition to the already configured Checkout, Subscription Read, Invoice Read, Product/Price Read, and Customer Portal permissions. Use any dependency permissions required by Stripe's key editor. No new secret or environment variable is needed.
2. Keep the portal cancellation mode **At end of billing period** and ordinary portal plan updates disabled. While a change is unresolved, **Manage billing** opens the payment-method-update flow; cancellation must wait until the change is undone or completed.
3. Deploy normally. The deployment script automatically runs migration **20260914_0024**; do not apply it manually on the deployed environment. API, scheduler and research processes must run the compatible version. Existing SMTP/API billing-worker configuration handles notifications.
4. If a change shows `needs review`, inspect its saved request and Stripe subscription/schedule. For a stranded schedule that has not taken effect, an operator can release it in Stripe (not cancel the subscription), then refresh/retry the saved request. No unknown provider object is adopted as a Radar-authorized target. Keep transition history: rollback refuses to drop a populated change table because historical invoice verification depends on it.

Acceptance checks in the sandbox:

- Schedule a cheaper paid tier, select retained active digests, and verify current benefits remain until renewal; undo and verify the original price remains.
- Schedule monthly→annual and annual→monthly, verifying that the latter waits for annual renewal and that there is no immediate invoice/proration.
- Simulate a lost create/configure/release response and retry; confirm one schedule and one logical notification per event.
