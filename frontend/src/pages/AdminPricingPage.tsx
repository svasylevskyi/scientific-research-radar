...
type PriceInput = { model_name: string; version: string; input_per_million: string;
  cached_input_per_million: string; output_per_million: string; web_search_per_call: string; max_input_tokens: string };
type Price = Omit<PriceInput, "max_input_tokens"> & { max_input_tokens: number; id: number; created_at: string; is_current: boolean };
type PriceList = { items: Price[]; total: number };
const blank: PriceInput = { model_name: "", version: "", input_per_million: "", cached_input_per_million: "", output_per_million: "", web_search_per_call: "", max_input_tokens: "" };
const rates = [
...
export function AdminPricingPage() {
  ...
  function copy(row: Price) {
    setForm({ model_name: row.model_name, version: "", input_per_million: row.input_per_million,
      cached_input_per_million: row.cached_input_per_million, output_per_million: row.output_per_million,
  ...
  async function save(event: FormEvent) {
    ...
    try {
      await apiRequest("/admin/pricing", { method: "POST", body: { ...form, max_input_tokens: Number(form.max_input_tokens) } });
  ...
  return <Box><AppHeader /><Container component="main" maxWidth="lg" sx={{ py: { xs: 3, sm: 6 } }}>
    ...
    <Stack spacing={3}>
      ...
      <Paper variant="outlined" sx={{ p: 3, borderRadius: 3 }}>
        <Box component="form" id="pricing-form" onSubmit={save}>
          ...
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr" }, gap: 2 }}>
            ...
            {rates.map(([key, label]) => <TextField key={key} required label={`${label} (USD)`} type="number" value={form[key]} onChange={(e) => update(key, e.target.value)} inputProps={{ min: 0, step: "any" }} disabled={saving} />)}
          ...
          <Alert severity="info" sx={{ my: 2 }}>These are flat standard-tier estimates. Special tariffs, reported cache writes, and unsupported service tiers remain unpriced. This does not change OpenAI billing or set a spending limit.</Alert>
      ...
      <Paper variant="outlined" sx={{ p: 3, borderRadius: 3 }}>
        ...
        {loading ? <Typography role="status" sx={{ mt: 2 }}>Loading pricing…</Typography> : !list.items.length ? <Alert severity="info" sx={{ mt: 2 }}>No pricing configured. Usage is recorded, but monetary estimates remain unknown until you add a tariff.</Alert> : <TableContainer><Table size="small" aria-label="Pricing history"><TableHead><TableRow>{["Model / version", "Input", "Cached input", "Output", "Search / call", "Max input", ""].map((label) => <TableCell key={label}>{label}</TableCell>)}</TableRow></TableHead><TableBody>
          {list.items.map((row) => <TableRow key={row.id}>
            ...
            {rates.map(([key]) => <TableCell key={key}>${row[key]}</TableCell>)}
