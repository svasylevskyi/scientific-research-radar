...
export function ContactPage({ workspace = false }: { workspace?: boolean }) {
  ...
  // Resolve the destination before accepting edits that a redirect would discard.
  const formDisabled = sending || isInitializing;
  ...
  if (!workspace && user) {
    return <Navigate to={{ pathname: "/radar/contact", search: location.search, hash: location.hash }} replace state={location.state} />;
  }

  return <Box>{workspace ? <AppHeader /> : <MarketingHeader />}<Container component="main" maxWidth="sm" sx={{ py: { xs: 4, md: 7 } }}>
