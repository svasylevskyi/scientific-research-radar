...
The digest page can save one schedule per digest, with daily, weekly, monthly, or quarterly frequency, a first-run date/time, an optional exclusive end date/time, and the browser’s IANA time zone. Dates are stored as UTC instants; the saved summary uses the stored zone. Editing displays the dates in the current browser’s zone and saves that zone. Cancel discards unsaved changes.

This is a preferences-only preview: no timer, recurring job, automatic run, or email delivery is created. Past dates are permitted in this draft configuration; an end, if supplied, must be strictly after the start. Recurrence rules and future activation are not implemented.

Run `alembic upgrade head` from `backend` before starting this version (migration `20260907_0009`). Existing digest frequency values are preserved as nullable legacy metadata; new digests can omit frequency or send null. Legacy frequency never creates a schedule. A migration downgrade removes schedule preferences and substitutes weekly for null legacy frequencies.

Owner-authenticated `PUT /api/v1/digests/{digest_id}/schedule` creates or replaces the single saved schedule and returns the digest. GET/list digest responses include `schedule` or null; admin reads use their existing access restrictions. The request requires `frequency`, timezone-aware `starts_at`, and `time_zone` (IANA identifier); `ends_at` is optional or null. Schedule data is excluded from radar prompt snapshots. No external requests are required to save it.

Owner-authenticated `DELETE /api/v1/digests/{digest_id}/schedule` removes only the schedule and returns 204, including when no schedule remains. Digest details and run history are preserved. The edit form offers Delete schedule between Save and Cancel for existing schedules, with a confirmation dialog before sending the request.
